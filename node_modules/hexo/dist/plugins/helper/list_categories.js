...
function listCategoriesHelper(categories, options) {
    ...
    // Blacklist of URLs (key) which should be manually redirected to the given value. - GoF
    const url_redirect_list = {'categories/Library/' : '/library/index.html'};
    ...
    const hierarchicalList = (level, parent) => {
        ...
        prepareQuery(parent).forEach((cat) => {
            ...
            // Added a loop and dictionary to allow for redirects from the category tags away from the default pages, since in my Hexo site I have a page called Library and a tag called Library, and I don't want an extra category page for library posts.
            var pathVar = cat.path;
            for (const [key, value] of Object.entries(url_redirect_list)) {
                pathVar = (cat.path.includes(key)) ? value : cat.path;
            }
            ...
            result += `<a class="${className}-list-link${isCurrent ? ' current' : ''}" href="${hexo_util_1.url_for.call(this, pathVar)}${suffix}">`;
    ...
    const flatList = (level, parent) => {
        ...
        prepareQuery(parent).forEach((cat, i) => {
            ...
            result += `<a class="${className}-link" href="${hexo_util_1.url_for.call(this, pathVar)}${suffix}">`;
            result += transform ? transform(cat.name) : cat.name;
            if (showCount) {
